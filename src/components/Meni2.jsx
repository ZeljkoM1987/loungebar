    ...
    return (
        ...
        <div className="products">
            <div className="left-izbor">
                <div className="left-izbor-title"> <h2>KATEGORIJE</h2></div>
                <ul className="izbor"  >
                    {/*<li onClick={() => setMenuProducts(Jelovnik) & setObrok('Sva Jela')}>Sva jela</li> */}
                    <li onClick={() => (filterspec("Novo") & setObrok('Novo u ponudi'))}>Novo u ponudi</li>
                    <li onClick={() => (filter("Dorucak") & setObrok('Doručak'))}>Doručak</li>
                    <li onClick={() => (filter("Burger") & setObrok('Burgeri i Ćevapi'))}>Burgeri i Ćevapi</li>
                    <li onClick={() => (filter("carte") & setObrok('A la carte'))}>A la carte</li>
                    <li onClick={() => (filter("Piza") & setObrok('Pizza'))}>Pizza</li>
                    <li onClick={() => filter("SlanePalacinke") & setObrok('Slane Palačinke')}>Slane Palačinke</li>
                    <li onClick={() => filter("Meze") & setObrok('Meze i Plate')}>Meze i Plate</li>
                    <li onClick={() => (filterposno(true) & setObrok('Piza'))}>Posni Program</li>
                    <li onClick={() => filter("Pasta") & setObrok('Njoki i Paste')}>Njoki i Paste</li>
                    <li onClick={() => filter("Salata") & setObrok('Obrok salate')}>Obrok salate</li>
                    <li onClick={() => filter("Corbe") & setObrok('Čorbe')}>Čorbe</li>
                    <li onClick={() => filter("Salate") & setObrok('Salate Porcije')}>Salate Porcije</li>
                    <li onClick={() => filter("Slatko") & setObrok('Slatki Program')}>Slatki Program</li>
                    <li onClick={() => filter("Prilozi") & setObrok('Prilozi')}>Prilozi</li>
            ...
            <div className="list">
                ...
                {MenuProducts.map((product, i) =>
                    ...
                    <div className="product" key={i} style={{ backgroundImage: `url(${product.url})` }}>


                        {!product.url && <div className="name">
                            <h1>{product.title}</h1>
                            <p>{product.text}</p>
                            <p>{product.vel}</p>
                        </div>}
            ...
            <div className="list-desktop">
                ...
                {MenuProducts.map((product, i) => {
                    ...
                    return (product.url !== " ") && < div className="product" key={i} >


                        <img src={product.url} alt="product-image" className="product-img" />
                ...
                {MenuProducts.map((product, i) => {
                    ...
                    return (product.url === " ") && < div className="product-none" key={i} >


                        <img src={product.url} alt="product-image" className="product-img-none" />
